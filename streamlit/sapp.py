...
def complete():
    ...
    df1 = df[(df['Rooms'] <= (num)) & (df['Price (HUF)'] <= maxp) & (df['Price (HUF)'] >= minp) & (df['Size (m2)'] <= maxs) & ((df['Size (m2)'] >= mins))].sort_values(['Rooms', 'Price (HUF)'], ascending=[False, True]).reset_index(drop=True)
    if len(df1) == 0:
